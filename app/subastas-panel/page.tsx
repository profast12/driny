...
export default function SubastasPanel() {
  ...
  const crearSubasta = async () => {
    ...
    setForm({ nombre: '', descripcion: '', categoria: 'Electronica', precio_base: '', incremento_minimo: '5000', duracion_horas: '24', emoji: '🛍️' });
          ...
          <div style={{ animation: 'fadeIn 0.3s ease' }}>
            {misSubastas.filter(s => s.activa && new Date(s.tiempo_fin) > new Date()).length === 0 ? (
              <div style={{ backgroundColor: '#1a1a1a', borderRadius: '16px', padding: '60px', textAlign: 'center', border: '1px solid #2a2a2a' }}>
                ...
                <p style={{ color: '#666', marginBottom: '24px', fontSize: '14px' }}>Crea tu primera subasta y llega a miles de compradores</p>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '20px' }}>
                {misSubastas.filter(s => s.activa && new Date(s.tiempo_fin) > new Date()).map(s => (
                  <div key={s.id} className="card-hover" style={{ backgroundColor: '#1a1a1a', borderRadius: '16px', padding: '20px', border: '1px solid #2a2a2a', transition: 'all 0.2s', animation: 'glow 3s infinite' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '14px' }}>
                      <div>
                        <p style={{ fontWeight: 'bold', fontSize: '16px', marginBottom: '4px' }}>{s.nombre}</p>
                        <p style={{ fontSize: '12px', color: '#666' }}>{s.categoria}</p>
                      </div>
                      <div style={{ backgroundColor: '#22c55e22', color: '#22c55e', padding: '4px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: 'bold', animation: 'pulse 2s infinite' }}>
                        EN VIVO
                      </div>
                    </div>
                    <div style={{ backgroundColor: '#111', borderRadius: '10px', padding: '14px', marginBottom: '14px' }}>
                      <p style={{ fontSize: '11px', color: '#666', marginBottom: '4px' }}>PRECIO ACTUAL</p>
                      <p style={{ fontSize: '22px', fontWeight: 'bold', color: '#f90' }}>${Number(s.precio_actual).toLocaleString('es-CO')} COP</p>
                      <p style={{ fontSize: '11px', color: '#666', marginTop: '4px' }}>Base: ${Number(s.precio_base).toLocaleString('es-CO')} COP</p>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '14px', fontSize: '13px' }}>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        <p style={{ color: '#666', fontSize: '11px' }}>TIEMPO RESTANTE</p>
                        <p style={{ fontWeight: 'bold', color: '#f90', fontSize: '13px' }}>{tiempoRestante(s.tiempo_fin)}</p>
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <a href={`/subasta/${s.id}`} target="_blank" className="btn-hover" style={{ flex: 1, textAlign: 'center', backgroundColor: '#f90', color: '#111', padding: '10px', borderRadius: '8px', textDecoration: 'none', fontWeight: 'bold', fontSize: '13px', transition: 'all 0.2s' }}>
                        Ver subasta
                      </a>
                      <button onClick={() => cancelarSubasta(s.id)} className="btn-hover" style={{ padding: '10px 14px', backgroundColor: '#1f1f1f', border: '1px solid #333', color: '#888', borderRadius: '8px', cursor: 'pointer', fontSize: '13px', transition: 'all 0.2s' }}>
                        Cancelar
                      </button>
                    </div>
                  </div>
          ...
          <div style={{ animation: 'fadeIn 0.3s ease', maxWidth: '600px' }}>
            <div style={{ backgroundColor: '#1a1a1a', borderRadius: '16px', padding: '32px', border: '1px solid #2a2a2a' }}>
              ...
              {[
                ...
                <div key={campo.key} style={{ marginBottom: '16px' }}>
                  ...
                  <input
                    ...
                    style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #2a2a2a', backgroundColor: '#111', color: 'white', fontSize: '14px', outline: 'none', boxSizing: 'border-box' }}
              ...
              <div style={{ marginBottom: '24px' }}>
                ...
                <textarea
                  placeholder="Describe el producto detalladamente: estado, caracteristicas, incluye..."
                  value={form.descripcion}
                  onChange={e => setForm(prev => ({ ...prev, descripcion: e.target.value }))}
                  rows={4}
                  style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #2a2a2a', backgroundColor: '#111', color: 'white', fontSize: '14px', outline: 'none', resize: 'vertical', boxSizing: 'border-box' }}
                  onFocus={e => e.target.style.border = '1px solid #f90'}
                  onBlur={e => e.target.style.border = '1px solid #2a2a2a'}
                />
              </div>

              <button onClick={crearSubasta} disabled={enviando} className="btn-hover" style={{ width: '100%', padding: '14px', backgroundColor: '#f90', color: '#111', border: 'none', borderRadius: '10px', fontWeight: 'bold', fontSize: '15px', cursor: enviando ? 'not-allowed' : 'pointer', transition: 'all 0.2s', opacity: enviando ? 0.7 : 1 }}>
                {enviando ? 'Creando subasta...' : 'Publicar subasta'}
              </button>
            </div>
          </div>
        )}
          ...
          <div style={{ animation: 'fadeIn 0.3s ease' }}>
            {misSubastas.filter(s => !s.activa || new Date(s.tiempo_fin) <= new Date()).length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {misSubastas.filter(s => !s.activa || new Date(s.tiempo_fin) <= new Date()).map(s => (
                  <div key={s.id} style={{ backgroundColor: '#1a1a1a', borderRadius: '12px', padding: '18px', border: '1px solid #2a2a2a', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <p style={{ fontSize: '11px', color: '#666', marginTop: '2px' }}>Precio final</p>
                    </div>
                  </div>
                ))}
              </div>
